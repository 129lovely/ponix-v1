...
def main(device_id, width, disp_width, disp_source, horizontal, num_sec):
	...
	with g.as_default(), g.device(device_t), tf.Session(config=soft_config) as sess:
		...
		while True:
			...
			# 화면 멈춤 stop
			# elif key_ == ord('s'):
			#     while key_ !='s' and key_ !=27:
			#         if key == 27:
			#             break s
			# 해당 화면의 결과 캡처 및 저장 capture
			elif key_ == ord('c'):
				dir_path = os.path.dirname(os.path.realpath('[plus]run_webcam_edit.py'))
				mk_path = 'capture'
				file_nm = 'screen_%s.png'%i
				if not(os.path.isdir('./capture')):
					os.makedirs(dir_path + "/" + mk_path)

				cv2.imwrite('./capture' + '/' + file_nm , output)
				print("%s is saved!"%file_nm)
				#name_i += 1
			...
			# 프린트 print
				...
				cv2.imwrite('./capture/print.png',output)
				os.system("lpr -P printer_name print.png") # 프린터 이름 쓰기
